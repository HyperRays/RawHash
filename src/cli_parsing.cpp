...
// set index and mapping options, initialize when preset is NULL
...
struct CLIParsedArgs {
	...
	ri_idxopt_t ipt;
	int n_threads = 3;
	char* idx_out_filename = 0; // where to dump index
	char* fpore = 0; // pore model filename
	FILE *fp_help = stderr; // where to print the help message
	int ru_server_port = -1; // for connecting to a real device via ReadUntil
	ketopt_t o; // for further parsing
};
// parse rawhash args
CLIParsedArgs parse_args(int argc, char *argv[]) {
	CLIParsedArgs parsed_args;
